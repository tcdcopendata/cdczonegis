...
// InfoWindow：群聚個案
var pointCase = function (req, res) {
    var client = new pg.Client(pgString);
    client.connect();

    var reqParameter = req.body.id.split(',');
    var sqlParameter = [];
    var strParameter = '';
    reqParameter.forEach(function (param) {
        if (strParameter != '') {
            strParameter += ',';
        }
        sqlParameter.push(parseInt(param));
        strParameter += '$' + sqlParameter.length.toString();
    });
    strParameter = ' WHERE "info_id" IN (' + strParameter + ')';
    var strSql = 'SELECT "REPORT","SICK_DATE","GENDER_DESC","SICK_AGE","LAT","LON"' +
        ' FROM "dengue_cluster_relation"' +
        ' LEFT JOIN "dws_report_dengue_gis"' +
        ' ON dengue_cluster_relation."case_report" = dws_report_dengue_gis."REPORT"' +
        strParameter + ' ORDER BY "SICK_DATE"';

    client.query(strSql, sqlParameter, function (err, row) {
        if (err) { console.log('err_group: ' + err) }
        client.end();
        var count_id = 0;
        var result = {};
...
// InfoWindow：環域分析
var pointArea = function (req, res) {
    var client = new pg.Client(pgString);
    client.connect();
    ...
    function getCircle(lat, lng, distance) {
        ...
        var options = { steps: 256, units: 'kilometers', properties: {} }
...
module.exports = { pointCase, pointArea };